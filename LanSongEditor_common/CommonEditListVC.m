...
{
    float speed=0.5f;  //0.5是放慢一倍; 2.0是加快一倍;
    LSOVideoEditor *ffmpeg=[[LSOVideoEditor alloc] init];
    WS(weakSelf)
    [ffmpeg setProgressBlock:^(int percent) {
        dispatch_async(dispatch_get_main_queue(), ^{
            
            int percent2=(int)(percent*speed);
            LSDELETE(@"percent is:%d",percent2)
            [weakSelf showProgress:percent2];
    ...
    [ffmpeg startAdjustVideoSpeed:srcVideoPath speed:speed];
}
